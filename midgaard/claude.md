...
│   │   │   ├── EdgarClient.kt
│   │   │   └── dto/
│   │   └── ovtlyr/                         # Ovtlyr.com buy/sell signal client (cookie-auth scrape)
│   │       ├── OvtlyrClient.kt            # Fetches raw payload via ovtlyr cookie credentials
...
│   │   ├── BadPrintIntegrityValidator.kt  # V1 (CRITICAL): bad-print V-shape bar (close >= 5x prev AND next <= 20% of spike) — canonical data-corruption signature. V2 (HIGH): split-adjustment failure with normal-priced prev (close >= 5x prev AND next >= 50% of close AND prev >= $0.01) — provider (EODHD) carried only a partial adjustment factor back, leaving a real corporate-action jump that the engine treats as a phantom return. V3 (HIGH): split-adjustment failure with sub-cent prev confirmed by real history (same shape as V2 but prev < $0.01 AND bar_position >= 5) — separates AT-class real low-price pre-split history from IHG-class stub-first-bar artifacts via the bar_position floor. V1/V2 apply the `MIN_PRICE` ($0.01) floor on prev to filter out stub bars at the symbol's listing date; V3 inverts the price floor but adds the bar-position guard.
