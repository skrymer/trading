...
- [[beta-delivery]] — a long book whose risk-adjusted return is just the index's; the failure mode G16 catches (seed — 4 confirmed screen-stage instances: [[george]] + PEAD's regime-tertile sign-flip thrice [raw gap + market-neutral residual + EPS-sign-gated residual]; the [[mrm]] instance is VOID pending re-run [ADR 0018 warmup artifact]; no G16-firewall instance yet).
...
- [[mrm]] — single-factor SPY-beta-stripped residual-momentum ranker; its /strategy-screen REJECT is VOID (warmup-starvation artifact, ADR 0018 — RNG-vs-RNG in OOS); single-factor recipe RE-OPENED, must re-run; multi-factor recipe (#137) built. See [[2026-06-09-trailing-ranker-warmup-starvation]].
...
- [[2026-06-09-trailing-ranker-warmup-starvation]] — engine defect: walk-forward loaded windows with no warmup buffer, so 504-day residual rankers were unscoreable in OOS → selection became tie-break RNG. Voids #130 (RNG-vs-RNG); fixed by ADR 0018; single-factor recipe re-opened.
