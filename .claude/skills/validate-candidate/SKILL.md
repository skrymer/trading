...
- **3-block separation** catches *edge decay* — the single most predictive failure mode for live deployment. A strategy whose alpha is shrinking across blocks will pass a 25y aggregate (because the early blocks are still strong) but fail block-separated comparison. The session's invalidated VCP, regime-gated MR3, and DV1 all showed this signature. 25y aggregate alone would miss it.
