...
- `spyBaselineComparison` (`SpyBaselineComparison?`) — SPY buy-and-hold **Calmar** baseline gate (ADR 0013): strategy stitched-OOS Calmar vs SPY's over the **identical** OOS support (SPY held only on the strategy's OOS days, stitched through the same per-window path). Fields: `verdict` (`PASS` / `FAIL` / `INCONCLUSIVE`), `strategyCalmar`, `benchmarkCalmar`, `benchmarkCagr`, `benchmarkMaxDrawdownPct`, `benchmarkSharpe`, `inconclusiveReason`. **Calmar-only** — Sharpe is reported (`benchmarkSharpe`) but not gated. `INCONCLUSIVE` (binds nothing, never auto-fails) when the stitched OOS series is < 60 trading days OR the **strategy's** stitched maxDD is < 3% (tiny denominator manufactures an explosive Calmar). `null` when the run is un-sized (no stitched strategy curve) or no SPY curve was available. The engine emits one verdict per run and is block-agnostic; binding scope (Block A / B / 25y aggregate binds, Block C informational) is enforced by `/validate-candidate`, not the engine.
...
- Same daily-bar / flat-`costBps` (cost modelled but liquidity-blind, mitigated by the default-on tradable-universe filter — ADR 0026) / survivorship-bias caveats as `/backtest` apply. WFE is a ratio of net OOS edge to net IS edge — cost nets out of both halves, so it barely moves WFE while it does lower the absolute OOS edge gate.
