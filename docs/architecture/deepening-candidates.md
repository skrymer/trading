...
| 2 | `DynamicStrategyBuilder` `when{}` factory | shallow abstraction (interface ≪ impl) | Medium | ✅ Completed (PR #11) — see [dynamic-strategy-builder-deepening.md](dynamic-strategy-builder-deepening.md). Transient parity fixture (`LegacyDynamicStrategyBuilder` + `ConditionRegistryParityTest`) removed in PR #28. **Open follow-up (doc hygiene):** that design doc still claims the fixture would be "deleted same PR" — add a closing note recording it was actually retired later, in PR #28. |
