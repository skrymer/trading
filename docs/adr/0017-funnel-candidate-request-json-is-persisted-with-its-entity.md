...
- **Wired into the skills (#140).** `/validate-candidate`'s `run-pipeline.sh` now persists its `$TEMPLATE` to `entities/<candidate>.request.json` **unconditionally** (every fired config — even a REJECTED one — gets re-tested when the engine/universe/baseline changes), and `/strategy-screen` persists a named candidate's screen request via the shared `.claude/scripts/persist-request-json.sh` helper. Persistence is no longer a manual copy step. (One canonical skeleton per *named* candidate — not per exploratory probe; a one-field variant like the Random baseline stays a documented edit in "Reproducing".)
- **Backfilled (#140).** The recoverable survivors are persisted: `btc-tyr` (Tyr's solo screen), `fenrir`, `baldr`, `order-block-breakout-condition`. These four died at *design-time* `/condition-screen`, so their persisted identity is a **conditions-screen** request (`/api/conditions/screen`), not a walk-forward backtest — each entity's "Reproducing" section carries the endpoint + universe caveat. Configs lost to retired dev docs (e.g. gjallarhorn's null) remain unrecoverable and are noted as gaps on their entities, not reconstructed.
