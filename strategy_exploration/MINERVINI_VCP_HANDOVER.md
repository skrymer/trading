...
**UPDATE 2026-06-03:** VCP-A/VCP-B **promoted** to first-class `NarrowingRangeCondition`(narrowingRange) + `VolumeDryUpCondition`(volumeDryUp) (TDD, deployed udgaard 1.0.81); **G14 `/verify-promotion` PASS** (946=946 trades, Jaccard 1.0, 0 divergences/25y — C14 satisfied). The **Component Firewall** is now **scoped + quant-reviewed + frozen + quant-signed** in `COMPONENT_FIREWALL_PLAN.md` (do NOT use v4 run-pipeline.sh — it false-rejects a regime component). **Next: FIRE the EX-ATR20×SSM block runs** (Block A/B/25y/C walk-forwards + per-block single backtests for days-in-market), then the calibrate-after checks against the frozen bars, then run EX-VCPOLD×SSM through the frozen gates.
