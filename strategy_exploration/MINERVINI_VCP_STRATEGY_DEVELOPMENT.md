...
_Status: **REJECTED at Component Firewall** (2026-06-03). EX-ATR20×SSM (promoted conditions, G14-PASS) failed 6 binding Component gates — C1a in-market CAGR 9.6–20.8% (≪ 30% floor, below even the 25% symmetric target) on all 3 blocks, plus 25y C2/C3/C5 and C7 on A+25y. **Failure mode: participate-and-lose in narrow-leadership chop — the breakout cousin of `feedback_mean_reversion_pullback_known_weakness`; `spyTrendUp` too coarse** (only stands aside in outright crisis, bleeds through 2015–16 / 2021–23 narrow-breadth tape with full 30–47-trade participation). EX-VCPOLD NO-GO (entry-population failure an exit can't fix). **Track 2 (add `breadthEma10Above50`) also REJECTED — WORSE (2026-06-03):** a scalar market-breadth gate did "thinning not selecting" — halved trades everywhere, flipped 3 good windows negative (2006/18/19), deepened bad windows (2011/2016), destroyed Block B's 0-negative proof (in-mkt CAGR 20.8→12.1). Law: a scalar market gate can't solve a cross-sectional per-name selection problem. **Track-2b (`sectorBreadthGreaterThanMarket`, per-name) ALSO REJECTED — kill rule FAILED (2011 deepened −4.6→−13.2, 2023 −19.4→−25.9; Block B thinned 152→106). STRIKE 3 → breakout-in-uptrend entry-time-regime premise DEPRECATED.** Three operator classes (no gate / scalar / per-name) all thin-and-deepen the same way → the failure is the entry premise, not the selector; no selector fixes it. **Breakout edge SHELVED as a known-real risk-on building block** (Block B earned it) for when a separately-validated regime-transition layer exists (issue #83). Forward: condition-ablation diagnostics (ADX-add, leave-one-out — design knowledge for a future candidate, no rescue) + universe/liquidity population-bias audit + pivot to a **defensive/crisis-specialist component**. Full results `COMPONENT_FIREWALL_PLAN.md` §10 + `TRACK2_BREADTH_GATE_PLAN.md` §8 + `TRADE_ANATOMY_ANALYSIS.md`. The 5 trend-template conditions + RS pipeline + promoted NarrowingRange/VolumeDryUp conditions (G14-PASS, unit-tested) are preserved, reusable capital._
