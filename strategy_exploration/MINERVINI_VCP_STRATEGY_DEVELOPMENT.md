...
_Status: **REJECTED at Component Firewall** (2026-06-03). EX-ATR20×SSM (promoted conditions, G14-PASS) failed 6 binding Component gates — C1a in-market CAGR 9.6–20.8% (≪ 30% floor, below even the 25% symmetric target) on all 3 blocks, plus 25y C2/C3/C5 and C7 on A+25y. **Failure mode: participate-and-lose in narrow-leadership chop — the breakout cousin of `feedback_mean_reversion_pullback_known_weakness`; `spyTrendUp` too coarse** (only stands aside in outright crisis, bleeds through 2015–16 / 2021–23 narrow-breadth tape with full 30–47-trade participation). EX-VCPOLD NO-GO (entry-population failure an exit can't fix). **Forward = Track 2: breadth-confirmed market gate as a NEW candidate re-screened from Stage 1, keeping the breakout premise + promoted conditions, replacing ONLY `spyTrendUp` — NOT a post-hoc patch on this config. Do not re-touch this config.** Full result in `COMPONENT_FIREWALL_PLAN.md` §10. The 5 trend-template conditions + RS pipeline + promoted NarrowingRange/VolumeDryUp conditions (G14-PASS, unit-tested) are preserved, reusable capital._
