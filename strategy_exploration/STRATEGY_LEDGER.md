...
| **Gjallarhorn** | Breadth-thrust **exhaustion-reversal** (crisis-bottom re-entry timer) | Zweig Breadth Thrust + capitulation reversal | **SHELVED — timing alpha CONFIRMED, composite A/B NO-GO (2026-06-04).** The gate `marketBreadthSustainedWashoutWithin(15,10,40) AND marketBreadthAbove(20) AND marketBreadthIncreasing(2)` fires on exactly 13 real crises/26y (0.50/yr, zero calm years, on the recovery), per-trade edge **+2.19%** beats all 20 conditional-within-regime null seeds **+22σ** (timing alpha real, NOT crisis beta). But the breakout+Gjallarhorn **offline two-curve blend** A/B is a **NO-GO**: full-period MAR +5% / DD +0% (benefit crisis-confined: Block B +31% MAR, Block C +46%/−7%DD) — all 3 pre-registered NO-GO triggers fired. **Structural:** a ~10%-active crisis leg as a fixed sleeve = cash drag; its crisis DDs overlap the breakout's (long-only ⇒ corr→1, ADR 0010) = no DD offset. Dynamic regime-switching (the only fix) stays triple-walled (abandoned regime portfolio). **Shelved as a certified-but-homeless matched pair with the breakout; no iteration.** Reusable: 3 washout conditions, the conditional-within-regime NULL methodology, the breadth-oscillator + 17-crisis-episode map. Full detail: `GJALLARHORN_STRATEGY_DEVELOPMENT.md` |
...
**Ranked screen order** (updated after George DEPRECATED + Forseti DISQUALIFIED + Gjallarhorn SHELVED): ~~George~~ (DEPRECATED §A) → ~~Forseti~~ (DISQUALIFIED — ~5y Ovtlyr data) → ~~Gjallarhorn~~ (SHELVED — timing alpha real, overlay A/B NO-GO §C.2) → **BTC + Tyr** (NEXT — breadth-thrust *continuation* + institutional-breakout + breadth-participation gate; quant-recommended 2026-06-04 as the strongest pick: adjacent to Gjallarhorn's proven breadth-state-timing alpha but active far more than ~10% of the calendar → fixes the cash-drag/CAGR-floor wall; screen *together*; scoping/design basis in `BTC_TYR_STRATEGY_DEVELOPMENT.md` — includes **ADX** (strongest unused breakout discriminator), breadth-as-thrust-not-level, and the **RSP/SPY** leadership-concentration signal pending ingestion #99) → **PEDQ** (re-scoped, after the earnings-data audit) → **Heimskringla** (gate on top of the best surviving picker; seasonality is ARS-fragile — lowest priority). **Mimir** drops to a low-priority cheap falsification only. **Absolute-momentum** = a gate applied to a survivor, never a standalone screen. **All ranker-selects candidates run the binding G-RANDOM baseline** (`BACKTESTING_FUNNEL.md` §2.2). **Carry-forward for BTC+Tyr (Gjallarhorn's lesson):** front-load the lottery-window gate + confirm cadence ≥1-2 deploy-windows/yr at the Step-0 firing-rate check *before* building bespoke, so a rare breadth-thrust gate doesn't inherit Gjallarhorn's standalone-unvalidatability.
