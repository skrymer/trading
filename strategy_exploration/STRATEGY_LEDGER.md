...
| **Gjallarhorn** | Breadth-thrust **exhaustion-reversal** (crisis-bottom re-entry timer) | Zweig Breadth Thrust + capitulation reversal | **TIMING ALPHA CONFIRMED (NULL test PASS, 2026-06-04); composite A/B PARKED on engine [#97](https://github.com/skrymer/trading/issues/97).** Standalone is un-validatable as a per-window strategy (cadence ceiling: ~13 crisis episodes/26y), so pursued as a crisis-reentry **overlay on the shelved breakout**. The correctly-shaped gate `marketBreadthSustainedWashoutWithin(15,10,40) AND marketBreadthAbove(20) AND marketBreadthIncreasing(2)` fires on exactly 13 real crises (0.50/yr, zero calm years, on the recovery) with per-trade edge **+2.19%**, which **beats all 20 conditional-null seeds by +22σ** (random same-regime dip-buying loses −0.17% — the timing rule carries the edge, NOT crisis beta). Next = breakout+Gjallarhorn **offline two-curve blend** A/B (go/no-go = MAR ≥1.20× & maxDD ≤0.85× breakout-alone, NOT the 30% floor — it's a defensive overlay), **blocked on #97** (daily MTM equity curve needed for blended maxDD). Three washout primitives built+deployed (`marketBreadthWashoutWithin` relative, `marketBreadthAbsoluteWashoutWithin` single-touch, `marketBreadthSustainedWashoutWithin` sustained — first two over-fired, third is the crisis gate). Full detail: `GJALLARHORN_STRATEGY_DEVELOPMENT.md` |
