...
**Walk-forward — long-history corroboration (1.25%, 22-window 36mo-IS / 12mo-OOS / 12mo step, 2000-2025):** WFE **1.152**, aggregate OOS edge **+3.61%** on 1,173 trades, **21/22 OOS-positive (95%)**, one negative OOS year (2022, −1.48%). Median per-window WFE **1.37** — most windows beat their in-sample edge. Fresh current-engine re-run 2026-05-22 (covers OOS years 2003-2024). Comes in below the prior documented figure (WFE 1.29 / +3.86% / 1,262 trades — a 23-window 2000-2026 run, 2026-05-02) — the prior run carried one extra OOS window (2025). Edge persistence holds across the full 25-year history; the single weak year is 2022, consistent with the strategy's documented 2022 drawdown.
...
*Dropped:*
- ~~Before-earnings exit (`beforeEarnings(1)`)~~ — tested 2026-05-17 against fresh 4-seed baseline (seeds 1/7/42/100, $10K, AtrRisk(1.25%, 2.0)). 4/4 seeds: earnings exit wins Calmar (+0.30 mean), MDD (−5.6pp), Sharpe (+0.04), Sortino (+0.31); 4/4 seeds: baseline wins CAGR (−8.12pp mean). Final capital $832K → $461K (−$371K over 10y). Verdict is objective-dependent — rejected under the plan's CAGR objective; would flip under a Calmar/Sortino objective. `BeforeEarningsExit` `@Component` stays registered for ad-hoc use. See `VCP_STRATEGY_DEVELOPMENT.md` section "Before-Earnings Exit Sweep".
