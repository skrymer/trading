...
**Regime:** `GET /api/regime/readout?after=&before=` (the daily 5-label regime read-out series over a window — THRUST/GRIND/NARROW/CHOP/CRISIS, `rawLabel` + dwell-debounced `publishedLabel`, ADR 0023), `GET /api/regime/current` (the latest available read as of the NY trading day; 404 when none). Read-out only — a strategy-blind market classifier, never wired into deploy/cash decisions (ADR 0010); strategies opt into it via the `regimeLabelIn` / `regimeLabelExit` conditions
