...
1. **Backtesting** (`backtesting/`)
   ...
   - `RegimeReadoutService.kt`: Pre-computes the pre-registered 5-label regime read-out (ADR 0023) — per trading day, which of the five canonical `RegimeLabel`s (THRUST/GRIND/NARROW/CHOP/CRISIS — CONTEXT.md "Market regimes") the tape is in, from three orthogonal market axes (breadth level+slope, the leadership-concentration gap, SPY realized vol) plus the sustained-washout crisis classifier; strategy-blind (no strategy result feeds it); `RegimeReadoutDaily` carries the instantaneous `rawLabel` + the dwell-debounced `publishedLabel` (both null on a fail-closed undefensible day) + the per-day `axes` (`RegimeAxes` — the raw breadth/gap/vol/washout readings the label was decided on; observability for the current-regime line + spec-calibration diagnostics, never gated on); pure `computeReadoutSeries` + impure `loadReadoutSeries` (warmup-loads SPY + the equal-weight aggregate + breadth over ~180 calendar days), mirroring `LeadershipRegimeService`; frozen pre-registered params in `RegimeReadoutParams.FROZEN`. `loadReadoutMapIfGated(entry, exit, after, before)` loads the series into `BacktestContext.regimeReadoutMap` (consumed via `getRegimeLabel(date)`, published label) only when a strategy gates on a regime label — recursing into nested condition groups — otherwise an empty map at zero cost. Strategies opt in via the `RegimeLabelCondition` (`regimeLabelIn`, entry gate, fail-closed on unlabeled days) and `RegimeLabelExitCondition` (`regimeLabelExit`, holds on unlabeled days). Surfaced over REST via `RegimeController` (`GET /api/regime/readout`, `GET /api/regime/current`). Read-out only — a market classifier, never wired into deploy/cash decisions (ADR 0010); the conditions are strategy-opt-in gates, not an engine-level switch.
