...
1. **Backtesting** (`backtesting/`)
   ...
   - `RegimeReadoutService.kt`: Pre-computes the pre-registered 5-label regime read-out (ADR 0023) — per trading day, which of the five canonical `RegimeLabel`s (THRUST/GRIND/NARROW/CHOP/CRISIS — CONTEXT.md "Market regimes") the tape is in, from three orthogonal market axes (breadth level+slope, the leadership-concentration gap, SPY realized vol) plus the sustained-washout crisis classifier; strategy-blind (no strategy result feeds it); `RegimeReadoutDaily` carries the instantaneous `rawLabel` + the dwell-debounced `publishedLabel` (both null on a fail-closed undefensible day); pure `computeReadoutSeries` + impure `loadReadoutSeries` (warmup-loads SPY + the equal-weight aggregate + breadth over ~180 calendar days), mirroring `LeadershipRegimeService`; frozen pre-registered params in `RegimeReadoutParams.FROZEN`. Read-out only — a market classifier, never wired into deploy/cash decisions (ADR 0010); endpoints/conditions are later phases.
