...
- **Midgaard** (standalone reference data service on port 8081) for OHLCV data with pre-computed indicators (ATR, ADX, EMAs, SMAs, Donchian, 52-week high/low, market-relative strength percentile), live quotes (Finnhub), options data, FX rates, ovtlyr buy/sell signals, and the treasury-yield reference series (gross 3-month T-bill via EODHD `US3M.GBOND`, served for idle-cash crediting — ADR 0016)
...
1. **Backtesting** (`backtesting/`)
   - `BacktestService.kt`: Core backtesting engine with capital-aware trade selection; records `EntryDecisionContext` on every trade (including skipped/missed) for post-hoc analysis; loads pre-window warmup history (`warmupLoadDate` from `StockRanker.warmupTradingDays()`) so trailing-history rankers can score early in-window entries — warmup bars are visible to ranker scoring only, never traded (gated to `[after, before]`; ADR 0018)
...
│   │   │   ├── mapper/               # StockMapper
│   │   │   ├── model/                # Stock (w/ ovtlyrSignals), StockQuote, OrderBlock, MarketBreadthDaily, SectorBreadthDaily, Earning, OvtlyrSignal, AssetType, EwReturnDaily (equal-weight cross-section feeding the leadership-gap regime, issue #83)
│   │   │   ├── repository/           # StockJooqRepository (incl. findEarnings(symbol) for ingestion fallback + findAllSymbolRecords() — the stocks-derived universe), MarketBreadthRepository, SectorBreadthRepository, LeadershipGapRepository (full-universe equal-weight 20-bar-return aggregate over the STOCK-or-null universe, ADR 0011; issue #83)
...
│   │   ├── integration/              # Provider abstractions + implementations (AlphaVantage, Massive, Finnhub, EODHD); self-rate-limiting; selection via ProviderConfiguration @ConditionalOnProperty; ovtlyr/ (OvtlyrClient + payload mapper for ovtlyr.com buy/sell signals); eodhd/ EodhdGovBondClient + TreasuryYieldMapper (3-month T-bill gross yield series, ADR 0016)
│   │   ├── service/                  # IngestionService, IndicatorCalculator, RateLimiterService, OvtlyrBackfillService, TreasuryYieldIngestionService, ApiKeyService, ScheduledIngestionService
│   │   ├── repository/               # jOOQ repositories (quotes, earnings, symbols, ingestion status, provider config, ovtlyr signals, treasury yields)
