...
- **Midgaard** (standalone reference data service on port 8081) for OHLCV data with pre-computed indicators (ATR, ADX, EMAs, SMAs, Donchian, 52-week high/low, market-relative strength percentile, gross-profitability quality percentile — ADR 0019), live quotes (Finnhub), point-in-time quarterly fundamentals (income-statement + balance-sheet line items incl. split-adjusted shares outstanding, ADR 0019), the point-in-time market-cap primitive inputs (un-adjusted raw close + split history, ADR 0027), options data, FX rates, ovtlyr buy/sell signals, and the treasury-yield reference series (gross 3-month T-bill via EODHD `US3M.GBOND`, served for idle-cash crediting — ADR 0016)
...
2. **Data** (`data/`)
   ...
   - Point-in-time market cap (ADR 0027): `Stock.marketCapAsOf(date)` = `(rawClose(date) / splitFactorAsOf(date)) × sharesOutstanding` — an in-memory accessor (no repository), split-only-adjusted close (`StockQuote.rawClose`, the un-adjusted provider close — `closePrice` is the dividend-AND-split-adjusted close and is the wrong leg for an absolute-level product) divided by the cumulative split factor k(t) (`Split` model), times the split-adjusted `Fundamental.sharesOutstanding` from the latest filing visible by `filingDate` (ADR 0019 gating)
   - `repository/LeadershipGapRepository.kt`: Full-universe equal-weight 20-bar-return aggregate (`ewReturnByDate`) over the point-in-time STOCK universe (null/unclassified asset_type held out, fail-closed; same population as breadth, ADR 0011/0026); feeds the leadership-gap regime
...
4. **Scanner** (`scanner/`)
   - `ScannerService.kt`: Scan for entry signals, check exits, validate entries against live quotes, CRUD trades, roll trades, close trades, drawdown stats; persists a `ScanRun` after each scan; gates entries to the tradable universe via `TradableUniverseFilter` always-on (no override — live trading never surfaces un-fillable names; same FROZEN constants as the backtest, the backtest≡scanner invariant, ADR 0026/#173); scan load window widened to `SCAN_LOOKBACK_DAYS=400` to satisfy the 252-bar age gate
...
**Conditions:** `POST /api/conditions/screen` (diagnostic condition pre-screen — forward-return lift, firing rate, ARS parameter sweep, SPY-regime lift, Jaccard overlap; firings + the all-bars baseline both gated to the tradable universe via the default-on `applyLiquidityFilter` per ADR 0026; `endDate` hard-capped at Block C's 2021-01-01 start per ADR 0007)
...
│   │   │   ├── mapper/               # StockMapper
│   │   │   ├── model/                # Stock (w/ ovtlyrSignals + fundamentals + splits; point-in-time TTM accessors grossProfitTtmAsOf / operatingMarginTtmAsOf / operatingMarginTtmPriorYearAsOf / latestFundamentalAsOf, ADR 0019; point-in-time market-cap accessors marketCapAsOf(date) / splitFactorAsOf(date), ADR 0027), StockQuote (w/ qualityPercentile + rawClose — un-adjusted close, the ADR 0027 absolute-level leg), OrderBlock, MarketBreadthDaily, SectorBreadthDaily, Earning, Fundamental (w/ sharesOutstanding, ADR 0027), Split (one split: symbol/exDate/ratio, the k(t) leg, ADR 0027), OvtlyrSignal, AssetType, EwReturnDaily (equal-weight cross-section feeding the leadership-gap regime, issue #83), LiquidityFilterParams (frozen pre-registered tradable-universe thresholds — close>=$5, 20d-median dollar-vol>=$1M, 252-bar min, $300M point-in-time market-cap floor (Phase 2, fail-open on a null cap — ADR 0026 §1/ADR 0027), STRESS_5M A/B variant; ADR 0026 — wired into the engine (default-on `applyLiquidityFilter` flag) + scanner (always-on, no override), #173)
│   │   │   ├── repository/           # StockJooqRepository (incl. findEarnings(symbol) + findFundamentals(symbol) + findSplits(symbol) for ingestion fallback + findAllSymbolRecords() — the stocks-derived universe, strict STOCK-typed only / null-asset_type held out fail-closed per ADR 0026), MarketBreadthRepository (strict STOCK-only breadth, ADR 0026), SectorBreadthRepository, LeadershipGapRepository (full-universe equal-weight 20-bar-return aggregate over the strict STOCK universe — null/unclassified held out fail-closed, ADR 0011/0026; issue #83)
│   │   │   └── service/              # StockService, StockIngestionService (resolveEarnings(symbol)/resolveFundamentals(symbol)/resolveSplits(symbol) fall back to the stored rows on provider failure — stale-but-present beats empty-because-we-failed; splits feed the ADR 0027 cap k(t)), TechnicalIndicatorService, OrderBlockCalculator, MarketBreadthService, SectorBreadthService, SymbolService, DataStatsService, ScheduledRefreshService, TradableUniverseFilter (point-in-time isEligible(stock, asOf) price/liquidity/age/cap gate over FROZEN LiquidityFilterParams — ADR 0026; cap leg fail-open on a null marketCapAsOf, ADR 0027; wired into BacktestService (3 co-resident entry sites) + WalkForwardService + ScannerService (live, always-on) via the default-on `applyLiquidityFilter` flag, #173)
...
│   ├── src/main/resources/           # Config, migrations (V1-V32)
...
│   │   ├── integration/              # Provider abstractions (incl. FundamentalsProvider, SplitsProvider — ADR 0027) + implementations (AlphaVantage, Massive, Finnhub, EODHD); self-rate-limiting; selection via ProviderConfiguration @ConditionalOnProperty (splits are EODHD-only, non-toggleable); ovtlyr/ (OvtlyrClient + payload mapper for ovtlyr.com buy/sell signals); eodhd/ EodhdFundamentalsClient (filtered fundamentals: General/Highlights/Earnings + Financials Income_Statement/Balance_Sheet quarterly incl. commonStockSharesOutstanding, ADR 0019/0027), EodhdSplitsClient (split history → k(t), parses "num/den" ratio strings, ADR 0027), EodhdGovBondClient + TreasuryYieldMapper (3-month T-bill gross yield series, ADR 0016)
│   │   ├── service/                  # IngestionService (upserts earnings + fundamentals + splits — splits replaced wholesale per symbol — in fetchAndSaveSupplementaryData; persists raw_close on each bar, ADR 0027), IndicatorCalculator, RateLimiterService, OvtlyrBackfillService, TreasuryYieldIngestionService, RelativeStrengthService, QualityPercentileService (operator-triggered cross-sectional GP/TA quality pass, ADR 0019), ApiKeyService, ScheduledIngestionService
│   │   ├── repository/               # jOOQ repositories (quotes — incl. recomputeQualityPercentiles SQL pass + raw_close, earnings, fundamentals — incl. shares_outstanding, splits (SplitRepository, replaceForSymbol, ADR 0027), symbols, ingestion status, provider config, ovtlyr signals, treasury yields)
...
│   │   ├── model/                    # Domain models (Models.kt — incl. Fundamental (w/ sharesOutstanding) + Quote.qualityPercentile + Quote.rawClose/RawBar.rawClose + Split (symbol/exDate/ratio, ADR 0027), OptionContractDto)
