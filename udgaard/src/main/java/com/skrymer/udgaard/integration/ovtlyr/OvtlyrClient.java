...
@Component
public class OvtlyrClient {
    private static String userId = "7273";
    private static String token = "Wryjy2eFJh_1106bff6-87c1-4d25-adb3-16fbe65c1322";
    
    public OvtlyrStockInformation getStockInformation(String symbol) {
        var restClient = RestClient.builder()
			.baseUrl("https://api.ovtlyr.com/v1.0/StockSymbol/GetAllDashboardChartBySymbolWithFiltersAndSort")	
			.build();
        var requestBody = "{\"stockSymbol\":\"%s\",\"period\":\"All\",\"page_index\":0,\"page_size\":20000}".formatted(symbol);
        ...
        return restClient.post()
			...
			.body(requestBody)
			.retrieve()
			.toEntity(OvtlyrStockInformation.class)
            .getBody();	
    }
     ...
     * @return
     */
    public OvtlyrMarketBreadth getMarketBreadth(String symbol){
        var restClient = RestClient.builder()
			.baseUrl("https://ovtlyr.com/market-breadth?handler=GetFormulaDashboardPlotValues")	
			.build();
        var requestBody = "{\"page_size\":2000,\"page_index\":0,\"period\":\"All\",\"stockSymbol\":\"%s\"}".formatted(symbol);
        ...
        return restClient.post()
			...
			.cookie("Token", "bHLj4DV1toLEOjxwBiStiIYkCod5Amw4b9qrVA9vixDkcCX8A+0pQZXWc/4qtw9FukzAuKKWO0xnDI/wI2b959/2MHHY6aRiQLut6rOSuce+G0cWhzjAtD4GnsVSqsC4")
			.header("ProjectId", "Ovtlyr.com_project1")
			.contentType(MediaType.APPLICATION_JSON)
			.body(requestBody)
			.retrieve()
			.toEntity(OvtlyrMarketBreadth.class)
            .getBody();	
    }
}
