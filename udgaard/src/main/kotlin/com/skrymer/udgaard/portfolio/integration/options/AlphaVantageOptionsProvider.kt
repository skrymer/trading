...
import com.skrymer.udgaard.data.integration.alphavantage.dto.AlphaVantageHistoricalOptions
...
class AlphaVantageOptionsProvider(
  ...
  override fun getHistoricalOptions(
    ...
    return runCatching {
      ...
      logger.error(
        "Failed to fetch historical options from AlphaVantage for $symbol${if (date != null) " on $date" else ""}: ${e.message}",
        e,
      )
    }.getOrNull()
  }
