...
class EtfStatsService(
    ...
    fun getEtfStats(etf: EtfSymbol, fromDate: LocalDate, toDate: LocalDate, forceFetch: Boolean = false): EtfStatsResponse {
        ...
        val allDates = stocks.flatMap { stock ->
            stock.quotes.filter { quote ->
                quote.date != null &&
                quote.date!! >= fromDate &&
                quote.date!! <= toDate
            }.mapNotNull { it.date }
        ...
        val allHistoricalData = allDates.map { date ->
        ...
        val warning = if (missingStockCount > 0) {
        ...
        val currentStats = if (completeHistoricalData.isNotEmpty()) {
    ...
    private fun calculateStatsForDate(stocks: List<Stock>, date: LocalDate): EtfHistoricalDataPoint {
        ...
        val bullishPercent = if (totalStocks > 0) {
