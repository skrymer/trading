...
class EtfService(
   ...
   */
  fun getEtf(symbol: String): EtfEntity? = etfRepository.findBySymbol(symbol.uppercase())
   ...
   * @return The ETF entity or null if not found
   */
  fun getEtf(symbol: EtfSymbol): EtfEntity? = getEtf(symbol.name)

  /**
   * Get all ETFs.
   * @return List of all ETF entities
   */
  fun getAllEtfs(): List<EtfEntity> = etfRepository.findAll()
   ...
   */
  fun etfExists(symbol: String): Boolean = etfRepository.existsBySymbol(symbol.uppercase())
