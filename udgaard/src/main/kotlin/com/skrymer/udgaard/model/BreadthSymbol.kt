...
sealed class BreadthSymbol {
     ...
     */
    data class Market(val symbol: MarketSymbol = MarketSymbol.FULLSTOCK) : BreadthSymbol() {
     ...
     */
    data class Sector(val symbol: SectorSymbol) : BreadthSymbol() {
        override fun toIdentifier(): String = symbol.name
        override fun toDescription(): String = symbol.description
    }
    ...
    companion object {
         ...
         */
        fun all(): List<BreadthSymbol> {
            return listOf(Market()) + SectorSymbol.entries.map { Sector(it) }
        }

        /**
         * Get all sector breadth symbols only.
         */
        fun allSectors(): List<BreadthSymbol.Sector> {
            return SectorSymbol.entries.map { Sector(it) }
        }
    }
}
