...
class EtfEntity {
   ...
   */
  fun getLatestQuote(): EtfQuote? = quotes.maxByOrNull { it.date }
   ...
   */
  fun getQuoteByDate(date: LocalDate): EtfQuote? = quotes.find { it.date == date }
   ...
   */
  fun isInUptrend(): Boolean = getLatestQuote()?.isInUptrend() ?: false
   ...
   */
  fun getHoldingsInUptrend(): List<EtfHolding> = holdings.filter { it.inUptrend }

  /**
   * Get the total weight of holdings in uptrend.
   * @return Sum of weights for holdings in uptrend
   */
  fun getTotalWeightInUptrend(): Double = holdings.filter { it.inUptrend }.sumOf { it.weight }

  override fun toString(): String = "EtfEntity(symbol=$symbol, name=$name, quotes=${quotes.size}, holdings=${holdings.size})"
}
