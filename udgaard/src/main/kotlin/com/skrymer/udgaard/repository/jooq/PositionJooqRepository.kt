...
import com.skrymer.udgaard.mapper.PositionMapper
...
class PositionJooqRepository(
   ...
   * Find position by ID
   */
  fun findById(id: Long): PositionDomain? {
    val position =
      dsl
   ...
   */
  fun findByPortfolioId(portfolioId: Long): List<PositionDomain> {
    val positions =
      dsl
        .selectFrom(POSITIONS)
  ...
  fun findOpenPositionByCharacteristics(
    ...
    val query =
      dsl
        ...
        .and(POSITIONS.STATUS.eq(PositionStatusDomain.OPEN.name))
   ...
   */
  fun save(position: PositionDomain): PositionDomain {
