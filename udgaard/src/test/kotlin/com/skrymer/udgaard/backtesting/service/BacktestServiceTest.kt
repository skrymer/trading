...
class BacktestServiceTest {
  ...
  fun `should use underlying asset for strategy evaluation and trading stock for P&L`() {
    ...
    val qqq = Stock("QQQ", "XLK", quotes = listOf(qqqQuote1, qqqQuote2, qqqQuote3, qqqQuote4))
    val tqqq = Stock("TQQQ", "XLK", quotes = listOf(tqqqQuote1, tqqqQuote2, tqqqQuote3, tqqqQuote4))
  ...
  fun `should handle mixed stocks with and without underlying assets`() {
    ...
    val aapl = Stock("AAPL", "XLK", quotes = listOf(aaplQuote1, aaplQuote2))
    ...
    val tqqq = Stock("TQQQ", "XLK", quotes = listOf(tqqqQuote1, tqqqQuote2))
    ...
    val qqq = Stock("QQQ", "XLK", quotes = listOf(qqqQuote1, qqqQuote2))
  ...
  fun `should limit total simultaneous open positions across multiple stocks`() {
    ...
    val stockAQuote4 = StockQuote(closePrice = 104.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 6))
    val stockAQuote5 = StockQuote(closePrice = 105.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 7))
    val stockAQuote6 = StockQuote(closePrice = 106.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 8))
    val stockAQuote7 = StockQuote(closePrice = 107.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 9))
    val stockAQuote8 = StockQuote(closePrice = 108.0, openPrice = 99.0, date = LocalDate.of(2025, 1, 10)) // Exit

    // Stock B: tries to enter on Jan 3 while Stock A is still open
    val stockBQuote1 = StockQuote(closePrice = 100.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 3))
    val stockBQuote2 = StockQuote(closePrice = 101.0, openPrice = 99.0, date = LocalDate.of(2025, 1, 6))
  ...
  fun `should allow new entry after open position exits when using maxPositions`() {
    ...
    val stockAQuote1 = StockQuote(closePrice = 100.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 1))
    val stockAQuote2 = StockQuote(closePrice = 101.0, openPrice = 99.0, date = LocalDate.of(2025, 1, 2)) // Exit
  ...
  fun `should allow multiple simultaneous positions up to maxPositions limit`() {
    ...
    val stockB =
      Stock("STOCK_B", "XLF", quotes = listOf(stockBQuote1, stockBQuote2, stockBQuote3, stockBQuote4))
    val stockC =
  ...
  fun `should have unlimited positions when maxPositions is null`() {
    ...
    val stockAQuote1 = StockQuote(symbol = "STOCK_A", closePrice = 100.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 1))
    val stockAQuote2 = StockQuote(symbol = "STOCK_A", closePrice = 101.0, openPrice = 99.0, date = LocalDate.of(2025, 1, 2))

    val stockBQuote1 = StockQuote(symbol = "STOCK_B", closePrice = 100.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 1))
    val stockBQuote2 = StockQuote(symbol = "STOCK_B", closePrice = 101.0, openPrice = 99.0, date = LocalDate.of(2025, 1, 2))

    val stockCQuote1 = StockQuote(symbol = "STOCK_C", closePrice = 100.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 1))
    val stockCQuote2 = StockQuote(symbol = "STOCK_C", closePrice = 101.0, openPrice = 99.0, date = LocalDate.of(2025, 1, 2))
  ...
  // ===== MARKET CONDITION DIAGNOSTICS TESTS =====
  ...
  fun `should capture market conditions using EMA-based SPY uptrend`() {
    ...
    Assertions.assertEquals(1, report.totalTrades)
    val condition = report.trades[0].marketConditionAtEntry
    Assertions.assertNotNull(condition, "Market condition should be captured when SPY is available")
    Assertions.assertTrue(condition!!.spyInUptrend, "Should use EMA-based isInUptrend(), not Ovtlyr spyInUptrend field")
    Assertions.assertEquals(500.0, condition.spyClose, 0.01)
  }

  @Test
  fun `should capture market breadth from calculated stock data`() {
    // Mock breadth data
    val breadthData = mapOf(
    ...
    Assertions.assertNotNull(condition!!.marketBreadthBullPercent, "Breadth should not be null")
    Assertions.assertEquals(65.0, condition.marketBreadthBullPercent!!, 0.01, "Should use calculated breadth value")
  }

  @Test
  fun `should include breadth and uptrend in market condition averages`() {
    // Set up breadth data for both trade entry dates
    val breadthData = mapOf(
      LocalDate.of(2025, 1, 1) to 60.0,
      LocalDate.of(2025, 1, 2) to 65.0,
    ...
    val spyQuotes = listOf(
      StockQuote(symbol = "SPY", closePrice = 500.0, trend = "Uptrend", date = LocalDate.of(2025, 1, 1)),
      StockQuote(symbol = "SPY", closePrice = 505.0, trend = "Uptrend", date = LocalDate.of(2025, 1, 2)),
    ...
    val quotes = listOf(
      StockQuote(symbol = "TEST", closePrice = 100.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 1)),
      StockQuote(symbol = "TEST", closePrice = 105.0, openPrice = 99.0, date = LocalDate.of(2025, 1, 2)),
      StockQuote(symbol = "TEST", closePrice = 100.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 3)),
      StockQuote(symbol = "TEST", closePrice = 95.0, openPrice = 99.0, date = LocalDate.of(2025, 1, 4)),
    )
    ...
    Assertions.assertEquals(2, report.totalTrades)
    val avgs = report.marketConditionAverages
    Assertions.assertNotNull(avgs)
    Assertions.assertTrue(avgs!!.containsKey("avgMarketBreadth"), "Should have avgMarketBreadth")
    Assertions.assertTrue(avgs.containsKey("spyUptrendPercent"), "Should have spyUptrendPercent")
    Assertions.assertFalse(avgs.containsKey("avgSpyHeatmap"), "Should NOT have avgSpyHeatmap")
    Assertions.assertEquals(65.0, avgs["avgMarketBreadth"]!!, 0.01, "Average of 60.0 and 70.0")
    Assertions.assertEquals(50.0, avgs["spyUptrendPercent"]!!, 0.01, "1 of 2 trades in uptrend = 50%")
  }

  @Test
  fun `should handle missing SPY gracefully`() {
    // Don't mock SPY - repository returns null by default
    val quote1 = StockQuote(closePrice = 100.0, openPrice = 100.0, date = LocalDate.of(2025, 1, 1))
    val quote2 = StockQuote(closePrice = 105.0, openPrice = 99.0, date = LocalDate.of(2025, 1, 2))
    val stock = Stock("TEST", "XLK", quotes = listOf(quote1, quote2))
