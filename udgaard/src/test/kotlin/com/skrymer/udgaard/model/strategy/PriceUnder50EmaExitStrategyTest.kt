...
class PriceUnder50EmaExitStrategyTest {

  @Test
  fun `price is under the 50ema at close`(){
    val priceUnder50EmaExitStrategy = PriceUnder50EmaExitStrategy()
    val exitQuote = validStockQuote()

    // given price is below 50ema at close
    exitQuote.closePrice = 9.9
    exitQuote.closePriceEMA50 = 10.0

    // then exit signal is true
    assertTrue(priceUnder50EmaExitStrategy.match(null, exitQuote))
  }

  @Test
  fun `price is over the 50ema at close`(){
    val priceUnder50EmaExitStrategy = PriceUnder50EmaExitStrategy()
    val exitQuote = validStockQuote()

    // given price is over 10ema at close
    exitQuote.closePrice = 10.0
    exitQuote.closePriceEMA50 = 9.9

    // then exit signal is true
    assertFalse(priceUnder50EmaExitStrategy.match(null, exitQuote))
  }
