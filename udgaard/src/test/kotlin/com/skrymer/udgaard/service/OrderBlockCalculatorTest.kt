...
class OrderBlockCalculatorTest {
    ...
    fun `should respect sensitivity thresholds`() {
        ...
        val highSensBlocks = calculator.calculateOrderBlocks(
            quotes = quotes,
            sensitivity = 28.0
        )

        // When: Calculate with low sensitivity (50 - does not detect 35% moves)
        val lowSensBlocks = calculator.calculateOrderBlocks(
            quotes = quotes,
            sensitivity = 50.0
        )
    ...
    fun `should prevent clustering of order blocks`() {
        ...
        for (i in 1 until bullishBlocks.size) {
            ...
            val daysBetween = java.time.temporal.ChronoUnit.DAYS.between(previous.startDate, current.startDate)
            assertTrue(
