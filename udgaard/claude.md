...
│   │   │   ├── RegimeReadoutService.kt   # Pre-registered 5-label regime read-out (ADR 0023): pure computeReadoutSeries (breadth level+slope / leadership-gap / SPY realized-vol decision table + sustained-washout CRISIS classifier → RegimeLabel; dwell-debounced publishedLabel; fail-closed null when undefensible) + impure loadReadoutSeries (warmup-loads SPY + equal-weight aggregate + breadth over ~180 calendar days); strategy-blind market classifier, NOT wired into deploy/cash (ADR 0010); endpoints/conditions are later phases
