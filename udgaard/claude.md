...
│   │   │   ├── RegimeReadoutDaily.kt # One day's regime read: instantaneous rawLabel + dwell-debounced publishedLabel (both null only on a genuinely-undefined day) + nested RegimeAxes (raw breadth/gap/median/IQR/vol/washout/252-bar-drawdown readings + advisory gapTrustworthy; observability only, never an input to a decision) (ADR 0023/0024)
│   │   │   ├── RegimeReadoutParams.kt # Frozen pre-registered read-out v2 spec (FROZEN); breadth/slope bands + asymmetric p33/p67 gap terciles + vol bands + dwell + washout + −20%/252-bar dd-CRISIS + advisory thin-N flag — changing a value is a methodology change, not a config knob (ADR 0023/0024)
...
│   │   │   ├── RegimeReadoutService.kt   # Pre-registered 5-label regime read-out v2 (ADR 0023/0024): pure computeReadoutSeries (breadth level+slope / median leadership-gap (asymmetric p33/p67 tercile bands) / SPY realized-vol decision table + two CRISIS legs (sustained-washout OR close-basis −20%/252-bar drawdown) → RegimeLabel; dwell-debounced publishedLabel; advisory thin-N trust flag (never fail-closed); null only on a genuinely-undefined day) + impure loadReadoutSeries (warmup-loads SPY + equal-weight aggregate + breadth over ~400 calendar days — the drawdown leg needs a trailing 252-bar high on day 1) + loadReadoutMapIfGated(entry, exit, after, before) (loads the BacktestContext.regimeReadoutMap only when a strategy gates on a regime label via getConditions() — recurses into nested condition groups — else empty at zero cost; BacktestService + WalkForwardService call it at context-build time, WF shares one window-independent series); strategy-blind market classifier, never an engine-level deploy/cash switch — strategies opt in via the regimeLabelIn/regimeLabelExit conditions on the gateable CRISIS/THRUST labels only + RegimeController surfaces it over REST (ADR 0010)
...
│   │           ├── entry/            # Entry conditions (discoverable via getAvailableConditions MCP tool); incl. RegimeLabelCondition (type regimeLabelIn, labels: stringList, category Market — permits entries only when the published read-out labels the day in the allowed set; only CRISIS/THRUST are gateable (GATEABLE_LABELS — GRIND/NARROW/CHOP rejected at build time), fail-closed on unlabeled days; ADR 0023/0024)
│   │           └── exit/             # Exit conditions; incl. RegimeLabelExitCondition (type regimeLabelExit, labels: stringList, category Signal — exits when the published label is in the exit set; only CRISIS/THRUST gateable (rejected at build time otherwise), holds on unlabeled days; ADR 0023/0024)
│   ├── data/                         # Data domain
│   │   ├── controller/
│   │   │   ├── StockController.kt
